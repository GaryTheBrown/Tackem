...
class ScraperRipper(Scraper):
    ...
    def javascript(self) -> str:
        """index of scraper"""
        java_file = str(
            open(os.path.dirname(__file__) + "/javascript/ripper.js", "r").read()
        )
    ...
    def __search_for_movie(
        ...
        page_range = (
            str(low_count) + " to " + str(max_count) + " of " + str(total_results)
        )
        ...
        for index, item in enumerate(response["results"]):
            ...
            button = ghtml_parts.input_button_on_click(
                "CHOOSE", "PopulateMovie(" + str(item["id"]) + ");"
            )
            full_original_language = Languages().get_name_from_2(
                item["original_language"]
            )
            body = html_parts.search_info(
                item["title"],
                item["original_title"],
                full_original_language,
                item["overview"],
                item["release_date"],
                item["poster_path"],
            ...
            accordian_cards += ghtml_parts.accordian_card(
                accordian_name, index, header, button, body
            )
    ...
    def __search_for_tvshow(self, query: str, page: int = 1) -> str:
        ...
        for index, item in enumerate(response["results"]):
            header = item["name"]
            button = ghtml_parts.input_button_on_click(
                "CHOOSE", "PopulateTVShow(" + str(item["id"]) + ");"
            )
            full_original_language = Languages().get_name_from_2(
                item["original_language"]
            )
    ...
    def __show_single_tvshow_item(self, item: dict) -> str:
        ...
        yes_button = ghtml_parts.input_button_on_click(
            "Yes", "PopulateTVShow(" + str(item["id"]) + ");"
        )
        no_button = ghtml_parts.input_button_on_click("No", "ClearModel();")
        footer = html_parts.yes_no_footer(yes_button, no_button)
        return json.dumps(
