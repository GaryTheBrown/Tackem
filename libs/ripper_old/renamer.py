...
class Renamer:
    ...
    def _video_renamer(self):
        ...
        msg1 = SQLSelect(INFO_DB, Where("ready_to_rename", True))
        Database.call(msg1)
        ...
        for item in msg1.return_data:
            ...
            for i, track in enumerate(rip_data.tracks()):
                ...
                if track.video_type() == "movie":
                    ...
                    out_file += (
                        "other - " + rip_data.name() + " - " + track.other_type()
                    )

                probe_video = FFprobe(
                    config["converter"]["ffprobelocation"].value, in_file
                ).get_video_info()[0]
            ...
            Database.call(
                SQLUpdate(INFO_DB, Where("id", item["id"]), ready_for_library=True)
            )
            if not self._thread_run:
                return
