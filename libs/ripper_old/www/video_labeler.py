...
class VideoLabeler(HTMLTEMPLATE):
    ...
    def single(self, index=None):
        ...
        data = (
            self._tackem_system.system()
            .get_video_labeler()
            .get_data_by_id(thread_name, index_int)
        )
        if data is False:
            self._return()
    ...
    def getids(self):
        ...
        return json.dumps(
            self._tackem_system.system().get_video_labeler().get_ids(db_label)
        )
    ...
    def edit(self, index=None):
        ...
        if file_location[0] != "/":
            file_location = (
                PROGRAMCONFIGLOCATION + config["locations"]["videoripping"].value
            )
        ...
        return self._template(
            edit_html, javascript=["scraper/ripper/javascript", "config_javascript"]
        )
    ...
    def editdisctype(self, index=None, disc_type_code=None):
        ...
        if disc_type_code == "change":
            self._tackem_system.system().get_video_labeler().clear_rip_data(
                thread_name, index_int
            )
    ...
    def _edit_disc_type_work(self, data, disc_type_code):
        ...
        if disc_type_code == "change":
            ...
            disc_type_html = html_parts.video_labeler_disctype_template(
                label, disc_type_code_label, rip_data, search
            )
    ...
    def _tracktype_section(self, disc_index, track_index, track_file, track_data=None):
        ...
        length = str(
            datetime.timedelta(seconds=int(format_info["duration"].split(".")[0]))
        )
        ...
        subtitle_count = (
            str(stream_counts["subtitle"]) if "subtitle" in stream_counts else "0"
        )
        ...
        track_panel = html_parts.video_panel(
            panel_head_html, "track_" + str(track_index), section_html
        )
    ...
    def edittracktype(self, disc_index=None, track_index=None, track_type_code=None):
        ...
        data = (
            self._tackem_system.system()
            .get_video_labeler()
            .get_data_by_id(thread_name, disc_index_int)
        )
        if data is False:
            self._return()
        ...
        track_file = (
            location + "/" + str(disc_index) + "/" + str(track_index).zfill(2) + ".mkv"
        )
        probe_info = FFprobe(config["converter"]["ffprobelocation"].value, track_file)
    ...
    def editsave(self, **kwargs):
            ...
            if len(array) <= 2:
                ...
                if array[2] != "stream":
                    ...
                    if "streams" not in data["tracks"][track_index]:
                        config = CONFIG["plugins"]["ripping"]["ripper"]
                        probe_info = FFprobe(
                            config["converter"]["ffprobelocation"].value,
                            file_dir + array[1].zfill(2) + ".mkv",
                        )
                        data["tracks"][track_index]["streams"] = [
                            None
                        ] * probe_info.stream_count()
        ...
        thread_name = "WWW" + cherrypy.request.remote.ip
        self._tackem_system.system().get_video_labeler().set_data(
            thread_name, kwargs["discid"], rip_data, finished
        )
        self._return()
