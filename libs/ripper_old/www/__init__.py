...
def mounts(
    key, instance_name=None
):  # take tackem_system off the line and generate it inside pass
    ...
    root.drives = Drives(
        "Ripper Drives", key, tackem_system, base_stylesheet=stylesheet
    )
    root.videolabeler = VideoLabeler(
        "Ripper Video Labeler", key, tackem_system, base_stylesheet=stylesheet
    )
    root.converter = Converter(
        "Ripper Video Converter", key, tackem_system, base_stylesheet=stylesheet
    )
    cherrypy.tree.mount(
        root,
