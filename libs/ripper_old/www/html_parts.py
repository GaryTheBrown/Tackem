...
def drives(drive_list, config_drives, vertical=False):
    ...
    for drive_index, drive_obj in enumerate(drive_list):
        ...
        drives_html += drive(
            drive_obj, drive_index, config_drives[cfg_name]["name"], vertical
        )
    return drives_html
...
def video_labeler_item(data, baseurl, vertical=False):
    ...
    disc_type_img = (
        baseurl + "ripping/ripper/static/images/" + data["disc_type"] + "-video.png"
    )
...
def video_labeler_tracktype_start():
    ...
    for d_item in video_track_type.TYPES:
        item_html += video_labeler_tracktype_start_item(
            d_item, video_track_type.TYPES[d_item], magic
        )
...
def _ffprobe_info_panel(data):
    ...
    for key in data:
        html += (
            '<div class="col-sm-4 col-6 border-right font-weight-bold">'
            + key
            + "</div>"
        )
...
def converter_item(data):
    ...
    if data["converting"]:
        ...
        progress_bar = html_parts.progress_bar(
            progress_string, data["process"], data["count"], data["percent"]
        )
